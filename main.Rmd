...
knitr::opts_chunk$set(echo=FALSE, warning=FALSE, message=TRUE, error=FALSE, cache=TRUE, cache.lazy = FALSE, tidy=TRUE, highlight=TRUE, collapse=TRUE, fig.fullwidth=TRUE, fig.align="center", fig.width=10)
```
...
```{r}
temp = as.data.frame(t(sapply(dplyr::select(test_eval_df_baseline, starts_with("MAE_")), summary)))
...
```{r}
temp = as.data.frame(t(sapply(dplyr::select(test_eval_df_h2o, -WARD_NAME, -MAE_weeklyNaive), summary)))
...
```{r}
temp = as.data.frame(t(sapply(dplyr::select(test_eval_df_arima, -WARD_NAME, -MAE_weeklyNaive), summary)))
